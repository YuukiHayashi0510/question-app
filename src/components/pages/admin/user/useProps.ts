...
import { deleteStoreUser, updateStoreUser } from '~/lib/Firebase/firestore'
import { useAdminUser } from '~/lib/hooks/useAdminUser'
import { Role, columns } from '~/types/user/const'
import type { Props } from './Layout'
import type { ModalValue } from '~/types/admin'
...
export function useProps(): Props {
  ...
  const onClickUpdate: Props['modalProps']['onClickUpdate'] = async () => {
    ...
    await updateStoreUser(modalValue['id'] as string, {
      id: modalValue.uid,
      name: modalValue.name,
      role: modalValue.role,
    })
      .then(() => alert(t('Alert.Admin.update')))
      .catch((e) => alert(e))
