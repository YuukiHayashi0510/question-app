...
import { useQuestionList } from '~/lib/hooks/useQuestion'
...
export function useProps(): Props {
  ...
  useEffect(() => {
    const f = async () => {
      ...
      setIsLoading(true)
      const maxSize = await getQuestionNum()
  ...
  const onClickMore = async () => {
    setIsLoading(true)
    const snapshot = await getQuestionMore(questionList.length + 3)
  ...
  const onClickDelete: Props['cardProps']['onClickDelete'] = async (idx) => {
    ...
    await deleteQuestion(questionList[idx]['id'] as string)
